...
class DeseretOCRDataset(Dataset):
    FONT_PATHS = [
        "/home/cayjobla/OCRTranslator/Noto_Sans_Deseret/NotoSansDeseret-Regular.ttf",
    ]
    ...
    def __init__(
        ...
        self._data = data
        self.translator = english_to_deseret
        ...
        self._data = self._data.map(
            self._preprocess_data, batched=True, batch_size=100
        )
    ...
    def _preprocess_data(self, batch):
        ...
        for example in batch['text']:   # Split each example, translate, and render text
            ...
            if len(words) > 0:
                ...
                translated_chunks.append(self.translator(chunks[-1], local=True))
                images.append(text_to_image(translated_chunks[-1], font=self.font))
        
        return {'text': chunks, 'deseret': translated_chunks, 'image': images}
...
def train(
    ...
    # train_dataset = DeseretOCRDataset(
    #     dataset["train"], processor, max_target_length=max_length
    # )
    # val_dataset = DeseretOCRDataset(
